...
class MainView extends React.Component {
    ...
    async componentDidMount() {
        await this.apiService.getNews({ limit: 16 }).then((data) =>{
            if(!data) return;
            this.setState({ news: data.results || [] , next: data.next, source: this.state.source});
        });
        await this.apiService.getTags().then((data) =>{
            if(!data) return;
    ...
    async handleTagOnClick(index, text) {
        await this.apiService.getNews({ 
            limit:16, 
            tags: text, 
    ...
    async handleTagSubmit(event) {
        if(event.keyCode === 13) {
            ...
            await this.apiService.getTags({ name: input }).then((data) =>{
                if(!data) return;
                this.setState({ tags: data || [] });
            });
        }
        ...
        if(this.state.next) {
            await this.apiService.getNews({
                ...
                this.setState(prevState => ({
                    news: [...prevState.news, ...data.results],
                    next: data.next
                }));
            });
            window.scrollBy({
